...
async def startup_event():
    ...
    topic = NewTopic(
        os.environ.get("TOPICS_PEOPLE_AVRO_NAME"),
        num_partitions=int(os.environ.get("TOPICS_PEOPLE_AVRO_PARTITIONS")),
        replication_factor=int(os.environ.get("TOPICS_PEOPLE_AVRO_REPLICAS"))
    )
    try:
...
def make_producer() -> SerializingProducer:
    ...
    Specifically a SerializingProducer
    """
    ...
    avro_serializer = AvroSerializer(
        ...
        lambda person, ctx: person.dict() # callable to convert Person model to dict
    )

    # Return serializing producer
    return SerializingProducer(
        {
            ...
            'partitioner': 'murmur2_random' # specific hashing algorithm for partitioning
        }
    )
